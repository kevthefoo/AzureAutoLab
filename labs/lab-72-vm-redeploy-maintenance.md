...
- [ ] **Task 4:** Create a maintenance configuration named `mc-weekend-window` with a scheduled window on Saturdays at 2:00 AM (4-hour duration) and assign it to `vm-app-01`
...
| 4   | Maintenance configuration assigned | Home > Maintenance Configurations > mc-weekend-window | Shows Saturday 2:00 AM window, 4-hour duration, `vm-app-01` assigned |
...
MC=$(az maintenance configuration show --resource-name mc-weekend-window -g "$RG" --query name -o tsv 2>/dev/null)
