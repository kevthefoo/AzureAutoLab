...
if [ -z "$KEY" ]; then
  ...
  if [ "$CN" = "finance-reports" ]; then echo "[PASS] Task 1: container finance-reports exists on $SA"; PASS=$((PASS+1));
  else echo "[FAIL] Task 1: container finance-reports missing on $SA"; FAIL=$((FAIL+1)); fi
  ...
  SK=$(az storage account show -n "$SA" -g "$RG" --query allowSharedKeyAccess -o tsv 2>/dev/null)
