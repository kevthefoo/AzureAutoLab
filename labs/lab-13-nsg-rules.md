...
- [ ] **Task 1:** Create an **Application Security Group** named `ASG-WebServers` in **East US** inside resource group `RG-Dev-Lab`
- [ ] **Task 2:** Add an **inbound security rule** to your existing NSG that allows **HTTPS (port 443)** traffic from the internet, priority **200**, named `Allow-HTTPS`
- [ ] **Task 3:** Add an **inbound security rule** that denies **all inbound traffic** with priority **4000**, named `Deny-All-Inbound`
