...
export default function SetupWizard() {
  ...
  const refresh = useCallback(async (fresh = true) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/setup/check${fresh ? "?fresh=1" : ""}`, {
        cache: "no-store",
      });
  ...
  async function switchSubscription(id: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Subscription switch failed");
    } finally {
      setBusy(null);
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-secondary text-sm mt-1">
          Quick prerequisites check. The app needs Azure CLI installed and signed in
          before any lab can run.
        </p>
      </div>
      ...
      <section className="bg-bg-surface border border-border rounded-lg p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-semibold">1. Azure CLI installed</h2>
            <p className="text-text-secondary text-sm mt-1">
      ...
      <section className="bg-bg-surface border border-border rounded-lg p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <h2 className="font-semibold">2. Logged in with a subscription</h2>
            <p className="text-text-secondary text-sm mt-1">
              {r ? r.azLogin.message : "Checking…"}
            </p>

            {data && data.subscriptions.length > 1 && r?.azLogin.ok && (
              <div className="mt-3">
                <label className="text-xs text-text-secondary block mb-1">
                  Switch subscription
                </label>
                <select
                  defaultValue={r.azLogin.subscription?.id || ""}
      ...
      <section className="bg-bg-surface border border-border rounded-lg p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-semibold">3. Can create/update resources</h2>
            <p className="text-text-secondary text-sm mt-1">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <h2 className="font-semibold">
              4. OpenAI API key{" "}
              <span className="text-text-secondary/60 text-xs font-normal">(optional)</span>
            </h2>
            <p className="text-text-secondary text-sm mt-1">
            ...
            <p className="text-xs text-text-secondary/60 mt-2">
              Written to <code className="font-mono">web/.env.local</code>. Restart{" "}
              <code className="font-mono">npm run dev</code> after saving so the chat
              picks it up.
            </p>
          </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <button
          ...
          disabled={!!busy}
          className="bg-bg-surface hover:bg-border border border-border text-text-primary text-sm px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
        >
          {busy || "Recheck"}
        </button>
        <button
          ...
          className="bg-accent hover:bg-accent-hover disabled:opacity-40 text-white text-sm px-5 py-2 rounded-lg transition-colors"
        >
          {completing ? "Finishing…" : ready ? "Continue to the app" : "Finish steps 1–3 first"}
        </button>
      </div>
    </div>
  );
